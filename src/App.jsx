  ...
  return (
    <>
      <div className="container">
        <h2 className='text-start'>產品列表</h2>
        <table className="table table-striped table-hover">
          ...
          <tbody>
            {products.map((item) => (  /* 使用map回傳陣列 */
              <tr key={item.id}>
                <td>{item.category}</td>
                <td>{item.origin_price}</td>
                <td>{item.price}</td>
                <td>{item.is_enabled ? '啟用' : '未啟用'}</td> {/* 判斷是否有資料，左邊是true，右邊是false的結果 */}
                <td><button type='button' className='btn btn-outline-primary' onClick={() => setTempProduct(item)}>查看細節</button></td> {/* 點擊按鈕時，將產品資料設定到tempProduct */}
              </tr>
            ))}
          </tbody>
        </table>
        <div>
          <h2 className='text-start'>單一產品細節</h2>
          {tempProduct ? ( /* 判斷tempProduct是否有資料，有的話顯示Card，沒有的話顯示文字 */
